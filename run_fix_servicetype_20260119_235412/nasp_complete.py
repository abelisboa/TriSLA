...
class NASPService:
    ...
    async def call_bc_nssmf(
        ...
        """
        (4) BC-NSSMF – Registro Blockchain REAL com resiliência
        ...
        - Enviar ao BC-NSSMF via POST /api/v1/register-sla
        
        Implementa retries com backoff para lidar com:
        - Flutuações do kubectl port-forward
        - Spikes momentâneos de latência
        - Falhas temporárias (ex.: Besu reiniciando)
        ...
        # Construir estrutura SLA-aware conforme Capítulo 6 (fora do loop de retry)
        ...
        # Preparar request para BC-NSSMF conforme API: POST /api/v1/register-sla
        ...
        # Retries com backoff (mínimo 3 tentativas conforme especificação)
        max_retries = 3
        timeout = 5.0  # Timeout adequado (componente connect + read)
        ...
        for attempt in range(max_retries):
            try:
                async with httpx.AsyncClient() as client:
                    # Enviar ao BC-NSSMF
                    response = await client.post(
                        f"{settings.bc_nssmf_url}/api/v1/register-sla",
                ...
                last_exception = e
                error_msg = f"BC-NSSMF: erro de requisição"
                logger.warning(f"⚠️ {error_msg} (tentativa {attempt + 1}/{max_retries}): {str(e)}")
        ...
        if last_exception:
            raise HTTPException(
                ...
                detail={
                    ...
                    "detail": f"Falha ao contatar o módulo BC-NSSMF após {max_retries} tentativas. Último erro: {str(last_exception)}"
                }
            )
    ...
    async def submit_template_to_nasp(self, nest_template: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                service_type = (
        # FIX: normalizar service_type de template_id
        template_id = nest_template.get("template_id") if isinstance(nest_template, dict) else None
        service_type = _normalize_service_type(template_id, service_type) if "_normalize_service_type" in dir() else service_type
        if not service_type or service_type == "automatic":
            raise ValueError("invalid_service_type: template_id/service_type inválido. Use template:URLLC, template:eMBB ou template:mMTC")
