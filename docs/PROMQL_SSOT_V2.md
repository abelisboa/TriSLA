...
**Specialized reference:** [](modules/telemetry.md) is the operational telemetry entry point; this file remains the PromQL-specific reference.  
...
**Tipo:** **derivada / proxy** no caminho UE–UPF–free5GC (exporter `trisla-ran-ue-upf-proxy`); simulador listado abaixo só para laboratório.  
...
|----------------|-------------------|-----|
| PRB | `avg(trisla_ran_prb_utilization{job="trisla-ran-ue-upf-proxy"})` | proxy (sim) / nativa (exporter) |
...
## Operational Telemetry Entry Point

Operational telemetry entry point: docs/modules/telemetry.md. This file remains the specialized PromQL reference for metric expressions and runtime query provenance.