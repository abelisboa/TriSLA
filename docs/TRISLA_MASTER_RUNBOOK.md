# [POST-REFACTOR FREEZE — FASES 2–6 / 2026-05-11] TriSLA arquitetura runtime oficial

> **SSOT.** A documentação operacional do TriSLA passou por uma refatoração de
> responsabilidades concluída em **FASE 6** (build/push/deploy por digest GHCR)
> e congelada em **FASE 7** (este bloco). Esta secção tem precedência semântica
> sobre os blocos históricos abaixo quando houver conflito de descrição de
> autoridade/ownership. Conteúdo histórico **não é apagado** para preservar a
> trilha de auditoria.

## Authority / Ownership atual

| Função                              | Autoridade                       | Executor             | Fallback                                                                                |
| ----------------------------------- | -------------------------------- | -------------------- | --------------------------------------------------------------------------------------- |
| Orchestration decision              | **Decision Engine** (FASE 3)    | declarativo          | Portal Backend legacy `ACCEPT→orchestrate` quando authority ausente                     |
| Orchestration execution             | NASP Adapter                     | NASP Adapter         | (status `nasp_orchestration_status=ERROR`, lifecycle pipeline composite mantido)        |
| Lifecycle authority (semantic)      | **Decision Engine** (FASE 4)    | declarativo          | Portal Backend `legacy-composer` quando authority ausente                               |
| Lifecycle pipeline state (operational) | Portal Backend (pipeline)     | Portal Backend       | inerente — composite `SUBMITTED/DECIDED/ORCHESTRATED/BC_*/COMPLETED`                    |
| Governance event (semantic)         | **Decision Engine** (FASE 4)    | declarativo          | Portal Backend `portal-legacy` quando authority ausente                                 |
| Governance registration (immutable) | **BC-NSSMF** (FASE 4)           | BC-NSSMF + Web3      | `governance_registration_fallback=true` em modo degraded                                |
| Runtime reassessment (revalidate-telemetry) | **SLA-Agent** (FASE 2)  | SLA-Agent            | Portal Backend `_revalidate_telemetry_local_fallback` quando env desligada/timeout      |

Detalhes completos em `evidencias_refactor_responsabilidades_20260511T191645Z/freeze/ownership/runtime_responsibility_matrix.md`.

## Imagens em runtime (digest-only)

| Serviço                | Digest GHCR (em produção 2026-05-11)                                          |
| ---------------------- | ----------------------------------------------------------------------------- |
| trisla-portal-backend  | `sha256:5629970f640120bb4c65eea4eea5b3f356f87ab66ce94b6a5cdf3f93f7283152` |
| trisla-decision-engine | `sha256:0658cd8b51bb424998c2de8edfe704ba1711225bc4451fec36b60eabc07db073` |
| trisla-sla-agent-layer | `sha256:7d0fa8d8bc8ff3c888e2696e525c7f9470bc613ebee0023a617b0c11498c789e` |
| trisla-bc-nssmf        | `sha256:57b79430d8b79803bc8c65e277597b58e0ace605c095bd45a3883847a7ec0099` |

Fonte: `evidencias_refactor_responsabilidades_20260511T191645Z/freeze/runtime/active_digests_summary.txt`.

## Política `digest-only`

- **Proibido** referenciar imagens por tag em manifestos/Helm/`kubectl set image`.
- **Obrigatório** capturar o digest via `podman push --digestfile`. `skopeo
  inspect` é a primeira tentativa; se devolver HTTP 403 (escopo do PAT
  insuficiente para bearer-token read), usar o digestfile como fallback
  determinístico.
- Pipeline reprodutível em `evidencias_refactor_responsabilidades_20260511T191645Z/deploy/PHASE6_REPORT.md`.

## SLA-Agent delegation (FASE 6 — PASSO 10)

O portal-backend tem em runtime:

```
SLA_AGENT_REVALIDATE_URL     = http://trisla-sla-agent-layer:8084/api/v1/agent/revalidate-telemetry
SLA_AGENT_REVALIDATE_ENABLED = true
SLA_AGENT_REVALIDATE_TIMEOUT_S = 8.0
```

Resposta esperada de `POST /api/v1/sla/revalidate-telemetry` contém
`metadata.delegated_to_sla_agent = true`. Em qualquer falha (`URL_unset`,
`disabled`, exceção HTTP), o portal cai para o fallback in-process e marca
`delegation_fallback_reason`.

Para **desligar** (rollback do PASSO 10):

```
ssh node006 'kubectl -n trisla set env deployment/trisla-portal-backend SLA_AGENT_REVALIDATE_ENABLED=false SLA_AGENT_REVALIDATE_URL-'
```

## Rollback procedure (resumo)

YAMLs pré-refatoração em
`evidencias_refactor_responsabilidades_20260511T191645Z/deploy/rollback/<deployment>_deployment_before.yaml`.

Rollback por **digest anterior** (preferido — não usa `:latest`) e por
`kubectl rollout undo` documentados em
`evidencias_refactor_responsabilidades_20260511T191645Z/deploy/rollback/ROLLBACK_REPORT.md`.

## Fallbacks preservados — NÃO REMOVER

Legacy fallback paths remain intentionally preserved after FASE 6 and **must
not be removed until dedicated cleanup validation is executed** (cleanup phase
futura, fora do escopo das FASES 0–7).

- `_revalidate_telemetry_local_fallback` no portal-backend (FASE 2).
- Caminho legado `ACCEPT→orchestrate` no `NASPService.submit_template_to_nasp`
  (FASE 3) quando `decision_engine_orchestration_authority` ausente.
- `lifecycle_state_source = portal_legacy_composite` quando DE-authority
  ausente (FASE 4).
- `governance_authority_source = portal-legacy` quando `governance_event`
  ausente (FASE 4).
- BC-NSSMF degraded path (`bc_enabled=false` ou RPC offline) com
  `governance_registration_fallback=true` (FASE 4).
...
## Closed-loop temporal (P0→P2) — SSOT operações

**Freeze consolidado:** `TEMPORAL_CLOSED_LOOP_P0_P2_FREEZE`  
**Estado e cadeia:** `analysis/CLOSED_LOOP_P0_P2_FINAL_STATUS.md`, `analysis/CLOSED_LOOP_EVOLUTION_CHAIN.md`  
**Metadados e contratos:** `analysis/RUNTIME_TEMPORAL_FEATURES.md`  
**Referências SSOT tocadas nesta consolidação:** `analysis/UPDATED_SSOT_REFERENCES.md`

### Evolução (resumo)

| Marco | O quê | Onde está documentado |
|--------|--------|------------------------|
| Q1 baseline | Imagem portal-backend + runtime estável | `docs/TRISLA_FINAL_RUNTIME_BASELINE_Q1.md`; nomes canónicos `evidencias_FINAL_RUNTIME_BASELINE_Q1_*`, `evidencias_Q1_RUNTIME_FREEZE_*` (pacotes sob `analysis/` quando não existirem pastas com esse prefixo na árvore) |
| P0 | `metadata.temporal_intent_trace` no `submit` | `analysis/P0_PATCH_APPLIED.md`, `analysis/P0_RUNTIME_EVIDENCE.json`, `analysis/p0_patch.diff` |
| P1 | `POST /api/v1/sla/revalidate-telemetry` + `drift_summary` | `analysis/P1_PATCH_APPLIED.md`, `analysis/P1_RUNTIME_EVIDENCE.json`, `analysis/p1_patch.diff` |
| P2 | `metadata.remediation_evidence` no retorno do revalidate (só declarativo) | `analysis/P2_PATCH_APPLIED.md`, `analysis/P2_RUNTIME_EVIDENCE.json`, `analysis/evidencias_TEMPORAL_REMEDIATION_EVIDENCE_P2_FREEZE_20260507T001000Z/` |

### Digests portal-backend (cadeia de referência)

Ordem **rollback** (mais recente → baseline Q1): P2 → P1 → Q1. Valores usados em ensaio documentado:

- **P2 (revalidate + remediation evidence):** `ghcr.io/abelisboa/trisla-portal-backend@sha256:80dbb97785290951a6c56b0c41b3f8b6648a3d244b50711341b0be4a93316e6a`
- **P1 (revalidate):** `ghcr.io/abelisboa/trisla-portal-backend@sha256:b7690cd9591be1017c20070926faccfd7ce93a73687892260757a8750c9439c4`
- **Q1 baseline:** `ghcr.io/abelisboa/trisla-portal-backend@sha256:38347a28b053cf2374b5bbc7c65ea76f168ce3b7ecea9699c52036213bef6b4d`

Confirmar sempre com `podman|docker pull …@sha256:…` antes de `kubectl set image` (alguns `RepoDigests` duplicados falham com `manifest unknown`).

### Endpoints

- `POST /api/v1/sla/submit` — inclui **`metadata.temporal_intent_trace`** (P0).
- `POST /api/v1/sla/revalidate-telemetry` — **`drift_summary`**, **`temporal_correlation`** (P1); **`metadata.remediation_evidence`** (P2, sem execução automática).

### Validação rápida

```bash
kubectl -n trisla port-forward svc/trisla-portal-backend <porta-local>:8001
curl -sS "http://127.0.0.1:<porta-local>/api/v1/health/global"
curl -sS -X POST "http://127.0.0.1:<porta-local>/api/v1/sla/revalidate-telemetry" \
  -H "Content-Type: application/json" \
  -d '{"intent_id":"smoke-closed-loop","reference_telemetry_snapshot":{}}'
```

Resposta esperada (P2): corpo com `drift_summary`, `metadata.remediation_evidence` com `recommendation`, `severity`, `affected_domains`, `suggested_action`, `revalidation_required`.
...
Consulta padrão (PromQL): **`avg(trisla_ran_prb_utilization{job="trisla-ran-ue-upf-proxy"})`**.  
Coleta no **portal-backend / NASP**: mesma série via `GET …/api/v1/prometheus/query` e env `TELEMETRY_PROMQL_RAN_PRB` quando definido (default alinhado a `avg(trisla_ran_prb_utilization{job="trisla-ran-ue-upf-proxy"})` — ver `apps/portal-backend` / contrato de telemetria).

Validação rápida (operador, port-forward):

```bash
kubectl -n monitoring port-forward svc/prometheus-kube-prometheus-prometheus 9090
curl "http://localhost:9090/api/v1/query?query=avg(trisla_ran_prb_utilization{job="trisla-ran-ue-upf-proxy"})"
```
...
# OFFICIAL_RUNTIME_FREEZE

Estado operacional oficial congelado:
...
Em caso de regressão futura:

1. retornar ao baseline Q1
2. validar Prometheus
3. validar I1
4. validar SLA submit
5. validar telemetry_snapshot
6. validar domain_actions

## Runtime Contracts

- portal-backend digest-only
- Prometheus validado
- I1 validado
- SLA runtime validado
- domain_actions validado
...
- `TELEMETRY_PROMQL_RAN_PRB='avg(trisla_ran_prb_utilization{job="trisla-ran-ue-upf-proxy"})'`
...
3. `curl -s "$PROMETHEUS_URL/api/v1/query?query=avg(trisla_ran_prb_utilization{job="trisla-ran-ue-upf-proxy"})"`
...
- **`apps/portal-backend/src/telemetry/collector.py`:** default documentado `avg(trisla_ran_prb_utilization{job="trisla-ran-ue-upf-proxy"})` quando `TELEMETRY_PROMQL_RAN_PRB` não está definida (override por env preservado).
...
Pacote documental: `docs/SSOT_RECOVERY/TRISLA_NASP_RUNTIME_BASELINE.md`, `docs/SSOT_RECOVERY/TRISLA_FINAL_FROZEN_BASELINE.md`, evidências `evidencias_nasp_alignment/` (Fases 1–3). **Fases 4–5 não executadas** — ver `evidencias_nasp_alignment/PHASE4_5_GATE.md` e `evidencias_nasp_alignment/fase3_plan/recovery_plan.md`.



## RAN SSOT governance (2026-05-13)

PromQL SSOT RAN PRB: **job `trisla-ran-ue-upf-proxy` only** (ver `docs/SSOT_RECOVERY/TRISLA_RAN_SSOT_POLICY.md`, `TRISLA_FALLBACK_POLICY.md`, `TRISLA_FINAL_RAN_GOVERNANCE_BASELINE.md`). Evidência pré-correção: `evidencias_ssot_ran_governance_20260513T204501Z/`.

---

## UPF_GTP5G_RUNTIME_RECOVERY

**Baseline freeze:** `evidencias_trisla_runtime_baseline_operacional_final_20260603T215746Z`  
**Evidence fix:** `analysis/upf_gtp5g_final_fix_20260603T214759Z/`  
**Verdict:** `UPF_GTP5G_PDU_ACCEPT_FINAL_FIX_APPROVED`
...
```
Wrong 5G GTP interface upfgtp
NetlinkSockOpen fail
Create PDR Error
```
...
```
gtp5g device named upfgtp created fail
Gtp5gDeviceAdd failed
```

Consequência downstream: SMF não emite N1N2 → UE sem PDU Accept → sem uesimtun0.

### Diagnóstico

No **node2** (UPF nodeSelector):

```bash
lsmod | grep gtp5g
ip link show upfgtp          # ausente = problema
dmesg | grep -i gtp5g | tail -20
```

Esperado se corrompido:
...
```bash
kubectl exec -n ns-1274485 upf-free5gc-upf-upf-0 -- ip link | grep upfgtp
kubectl logs upf-free5gc-upf-upf-0 -n ns-1274485 --tail=50 | grep -E "upfgtp|Gtp5g|PDR"
```

### Correção (ordem obrigatória)

**CRITICAL:** parar UPF **antes** de recarregar gtp5g.

```bash
# 1. Parar UPF
kubectl scale sts upf-free5gc-upf-upf -n ns-1274485 --replicas=0
kubectl wait --for=delete pod/upf-free5gc-upf-upf-0 -n ns-1274485 --timeout=120s

# 2. No host node2 (SSH ou nsenter)
sudo rmmod gtp5g
sudo modprobe gtp5g
...
# 3. Subir UPF
kubectl scale sts upf-free5gc-upf-upf -n ns-1274485 --replicas=1
kubectl wait --for=condition=Ready pod/upf-free5gc-upf-upf-0 -n ns-1274485 --timeout=180s

# 4. Sincronizar SMF nodeID se Pod IP mudou
UPF_IP=$(kubectl get pod upf-free5gc-upf-upf-0 -n ns-1274485 -o jsonpath='{.status.podIP}')
echo "UPF_IP=$UPF_IP"
# Atualizar smf-configmap: userplaneInformation.upNodes.UPF.nodeID + N3 endpoints
kubectl delete pod smf-free5gc-smf-smf-0 -n ns-1274485
```
...
```bash
kubectl exec -n ns-1274485 upf-free5gc-upf-upf-0 -- ip link | grep upfgtp
kubectl rollout restart deployment/ueransim-singlepod -n ueransim
kubectl logs deployment/ueransim-singlepod -n ueransim -c ue --tail=50 | grep -E "PDU Session Establishment Accept|uesimtun"
kubectl exec -n ueransim deployment/ueransim-singlepod -c ue -- ping -c 3 8.8.8.8
```

**Esperado:** upfgtp UP; PDU Accept; uesimtun0 10.1.0.1; ping OK.

### Notas

- Não recarregar gtp5g com UPF ativo — reinicia estado corrompido.
- UPF roda em node2 (`5.4.0-198-generic`); gtp5g v0.8.2 em `/lib/modules/.../gtp5g.ko`.
- Não alterar TriSLA, versões Free5GC, ou mascarar PDR fail.
