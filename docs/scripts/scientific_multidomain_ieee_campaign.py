...
IPERF_NAMESPACE = os.getenv("TRISLA_IPERF_UE_NAMESPACE", "ueransim")
...
class TrafficPoint:
    """Operational stress via real UE iperf — PRB emerges from proxy only (no /set)."""
...
# Progressive real-traffic pressure (low → moderate → sustained → congestion).
TRAFFIC_POINTS: list[TrafficPoint] = [
    TrafficPoint("regime1_baseline", "idle", None),
    TrafficPoint("regime1_baseline", "idle_2", None),
    TrafficPoint("regime1_baseline", "idle_3", None),
    TrafficPoint("regime2_moderate", "10M", "10M"),
    TrafficPoint("regime2_moderate", "20M", "20M"),
    TrafficPoint("regime2_moderate", "30M", "30M"),
    TrafficPoint("regime3_sustained", "50M", "50M"),
    TrafficPoint("regime3_sustained", "70M", "70M"),
    TrafficPoint("regime4_congestion", "90M", "90M"),
]
...
def _query_proxy_prb() -> Optional[float]:
    series = _query_raw_prb_series()
    return _safe_float(series.get("trisla-ran-ue-upf-proxy"))


def _apply_real_traffic_stress(
    iperf_bitrate: Optional[str],
    *,
    duration_s: int = 90,
) -> tuple[dict[str, Any], Optional[subprocess.Popen[bytes]]]:
    """Start real UE iperf load; PRB is observed from proxy SSOT only."""
    prb_before = _query_proxy_prb()
    proc: Optional[subprocess.Popen[bytes]] = None
    if iperf_bitrate:
        proc = _start_iperf(iperf_bitrate, duration_s=duration_s)
        time.sleep(IPERF_WARMUP_S)
    prb_after_warmup = _query_proxy_prb()
    meta = {
        "traffic_mode": "real_ue_iperf" if iperf_bitrate else "baseline_idle",
        "iperf_bitrate": iperf_bitrate or "none",
        "proxy_prb_before": prb_before,
        "proxy_prb_after_warmup": prb_after_warmup,
        "prb_proxy_promql": PRB_PROXY_PROMQL,
    }
    return meta, proc
...
def _write_report(df: pd.DataFrame, report_path: Path, extra: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- PRB SSOT: `{PRB_PROXY_PROMQL}` (real traffic via UE iperf)",
...
def main() -> int:
    ...
    for point in TRAFFIC_POINTS:
        stress_meta, proc = _apply_real_traffic_stress(point.iperf_bitrate)
        time.sleep(PRB_SCRAPE_WAIT_S)
        for rep in range(REPS_PER_PRB):
            tenant = f"{run_tag}-{point.regime}-{point.stress_label}-{rep}"
            result = _submit(tenant, point.regime)
            proxy_prb_submit = _query_proxy_prb()
            rows.append(
                _extract_row(
                    run_tag,
                    point.regime,
                    point.regime,
                    {
                        "stress_label": point.stress_label,
                        "iperf_bitrate": point.iperf_bitrate or "none",
                        "proxy_prb_at_submit": proxy_prb_submit,
                        **stress_meta,
                    },
                    result,
                )
    ...
    for profile, bitrate in IPERF_BITRATES.items():
        stress_meta, proc = _apply_real_traffic_stress(
            bitrate, duration_s=max(60, int((REPS_REGIME4 * 12) + 20))
        )
        time.sleep(PRB_SCRAPE_WAIT_S)
        for rep in range(REPS_REGIME4):
            ...
            rows.append(
                _extract_row(
                    run_tag,
                    f"regime4_{profile}",
                    "regime4_rtt_sweep_real_traffic",
                    {
                        "rtt_profile": profile,
