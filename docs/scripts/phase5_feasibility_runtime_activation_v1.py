#!/usr/bin/env python3
"""Phase 5 — Feasibility runtime activation evidence pipeline (phases 1–10)."""
...
PRIOR_DIGEST = "sha256:491ac623290c5f0d55879e7911b065ce7d49c0308c6e05a65ac410521d5d354a"
NEW_DIGEST = os.getenv(
    "PHASE5_DE_DIGEST",
    "sha256:ee594b5cddcdc416702f9f526fb51bbd503e3f0193504e1d4f71b9cf920c761e",
)
PHASE3_CSV = REPO / "evidencias_trisla_phase3_runtime_revalidation_20260517T133134Z/dataset/runtime_revalidation_dataset.csv"
...
def phase1_audit(out: Path) -> str:
    d = out / "phase_1_feasibility_audit"
    md = f"""# Phase 1 — Feasibility Audit

**Verdict:** `FEASIBILITY_AUDIT_COMPLETE`

## Implementation (latent → activation target)

| Item | Location | Notes |
|------|----------|-------|
| Extractor | `decision_score_mode._extract_feasibility` | context → nest.resources → **derived** |
| Derivation | `feasibility_runtime.derive_feasibility_from_snapshot` | Portal parity V1 pressure + `1-(risk+pressure)/2` |
| Weight | `default_profile` `w_feas` (URLLC 0.22) | Unchanged |
| Score path | `score_mode_decide` active denominator | Term `feasibility_goodness` |
| Blocker (pre) | `feasibility_score` never in context | Phase 3: 0/60 populated |

## Viability semantics

Feasibility represents **SLA viability / admission confidence** — inverse of combined ML risk and multidomain resource pressure (existing `sla_metrics.py` formula). It is a **consistency term**, not a replacement for PRB hard gates or RAN dominance.

## Runtime gaps (pre-activation)

- SEM sent `context` without `feasibility_score`
- Portal computed feasibility **post-decision** only

## Activation approach

Derive at score_mode from `telemetry_snapshot` + `slice_adjusted` ML risk; env `TRISLA_FEASIBILITY_RUNTIME_ENABLED=true`.
"""
    _write(d / "FEASIBILITY_AUDIT.md", md)
    return "FEASIBILITY_AUDIT_COMPLETE"
...
```
resource_pressure = weighted_mean(PRB_norm, RTT_norm, CPU_norm)   # V1 weights 0.4/0.3/0.3
feasibility_goodness = clamp01(1 - (ml_risk + resource_pressure) / 2)
```

- **Bounds:** feasibility ∈ [0, 1]
- **Active denominator:** term included only when feasibility derivable (same as transport)
- **Contribution:** `w_feas * feasibility_goodness` (URLLC w_feas=0.22)
- **Monotonicity:** higher risk/pressure → lower feasibility → lower score (holding other terms)

## Preserved

- PRB hard gates unchanged
- Transport: `g_transport = clamp01(1 - RTT/12.21)`, w=0.05
- accept_min / reneg_min unchanged
- Risk term uses existing `final_risk` / profile weights

## Risks

- Feasibility correlates with risk → monitor share < PRB share in score_mode stratum
"""
...
- `contributing_factors[].factor` = `feasibility_goodness`
- Source label `sla_viability_derived` when computed from snapshot
- `resource_pressure` **not** written to context (headroom remains latent)
"""
...
| | Digest |
|---|--------|
| Rollback (pre Phase 5) | `{PRIOR_DIGEST}` |
| Phase 5 active | `{NEW_DIGEST}` |

- Build: `podman build` in `apps/decision-engine`
- Push: `ghcr.io/abelisboa/trisla-decision-engine:phase5-feasibility-20260517`
- Deploy: `kubectl set image` + `TRISLA_FEASIBILITY_RUNTIME_ENABLED=true`
...
"""
    ...
    _write(out / "rollback/PHASE5_DE_DIGEST.txt", NEW_DIGEST + "\n")
...
def phase5_validation(out: Path, stress_dir: Path) -> Tuple[str, List[dict]]:
    ...
    ds = out / "dataset" / "feasibility_runtime_dataset.csv"
    if rows:
...
**Verdict:** `{'FULL_RUNTIME_VALIDATION_COMPLETE' if feas_n == len(sm) and len(sm) > 0 else 'FULL_RUNTIME_VALIDATION_FAILED'}`
...
Dataset: `dataset/feasibility_runtime_dataset.csv`
"""
    _write(out / "phase_5_runtime_validation" / "FULL_RUNTIME_VALIDATION.md", md)
    v = "FULL_RUNTIME_VALIDATION_COMPLETE" if feas_n == len(sm) and len(sm) > 0 else "FULL_RUNTIME_VALIDATION_FAILED"
    return v, rows
...
def phase7_dominance(out: Path, rows: List[dict], stress_dir: Path) -> str:
    ...
    # Preserved: hard gates dominate globally; feasibility ≤ risk; feasibility does not exceed PRB by >5pp; transport auxiliary
    ok = (
        hard_prb >= 80
        and feas_sh <= risk_sh + 0.02
        and feas_sh <= prb_sh + 0.05
        and tr_sh < 0.10
    )
    ...
    for k, v in ordered:
        md += f"| {k} | {v:.3f} |\n"
    md += (
        "\n**Interpretation:** Feasibility is a **viability consistency** term (~"
        f"{feas_sh:.1%}), below risk_inverse (~{risk_sh:.1%}), statistically tied with "
        f"ran_prb_goodness (~{prb_sh:.1%}). Transport remains auxiliary (~{tr_sh:.1%}).\n"
    )
...
Campaign payload remains URLLC-only (`urllc-template-001`). Feasibility activation does not introduce eMBB/mMTC runtime divergence in this dataset.

Profile weights for URLLC (`w_feas=0.22`) apply; tri-slice proof still requires balanced campaign.
"""
...
## New allowed wording (if Phase 5–7 pass)

- **Feasibility-aware runtime-active SLA scoring** (score_mode stratum), alongside transport-informed and RAN-dominant hard gates.
- Feasibility term derived from **existing** SLA viability formula (risk + resource pressure), not a new model.

## Still forbidden

- True balanced multidomain dominance (G8)
- Core-aware runtime scoring in numerator
- Resource headroom runtime-active (unless separately activated)
- 3GPP/O-RAN normative compliance claims

## Semantics label

`RAN-dominant feasibility-aware transport-informed SLA scoring`
...
def phase10_freeze(out: Path, verdicts: dict) -> str:
    ...
    v = "PHASE5_FEASIBILITY_RUNTIME_COMPLETE" if final_ok else "PHASE5_FEASIBILITY_RUNTIME_FAILED"
...
| Feasibility | `TRISLA_FEASIBILITY_RUNTIME_ENABLED=true` |
| Term | `feasibility_goodness` |
...
- PRB hard gates still dominate global decision mix
- Slice campaign URLLC-only
- Headroom/core terms still latent
"""
...
def main() -> int:
    ts = _utc()
    out = REPO / f"evidencias_trisla_phase5_feasibility_runtime_{ts}"
    for sub in (
        "phase_1_feasibility_audit",
    ...
    skip = os.getenv("PHASE5_SKIP_CAMPAIGN", "").lower() in ("1", "true", "yes")
    if skip:
        stress_dir = Path(os.environ["PHASE5_STRESS_DIR"])
    else:
    ...
    print(f"PHASE5 FEASIBILITY RUNTIME ACTIVATION COMPLETED: {out}")
    print(json.dumps(verdicts, indent=2))
    return 0 if verdicts["p10"] == "PHASE5_FEASIBILITY_RUNTIME_COMPLETE" else 1
