...
### Telemetry role in decision
- Decision-critical signal: **RAN load (modeled)** via PRB utilization.
- Core resource utilization (CPU/memory) is treated as contextual input and does not directly trigger SLA acceptance thresholds.
- Transport variability (runtime jitter) is not directly used in decision policy; it contributes indirectly through SLA-defined requirements processed by ML-NSMF.
...
## Runtime-aligned telemetry interpretation

- PRB is the direct policy anchor for decision pressure.
- Core resource utilization complements risk/context awareness.
- Transport variability (jitter) is monitored and exposed in telemetry snapshots, but is not a direct threshold variable in policy rules.
...
Para auditoria fim‑a‑fim com rastreio por `intent_id`, os logs do deployment `trisla-decision-engine` devem conter o identificador em cada decisão emitida. A campanha automatizada em `scripts/e2e/trisla_full_closure_campaign.py` correlaciona submits do Portal com `kubectl logs` nesse deployment (entre outros). Baseline de decisão: **Hybrid SLA-Aware Policy-Governed Decision Model** (sem alteração de política na campanha).
