...
class Intent(BaseModel):
    ...
    sla_requirements: Optional[SLARequirements] = Field(
        None,
        description="Requisitos de SLA (opcional, pode ser gerado internamente)"
    )
    intent_text: Optional[str] = Field(
        None,
        description="Texto em linguagem natural para processamento semântico"
    )
...
class IntentRequest(BaseModel):
    ...
    """
    service_type: Literal["URLLC", "eMBB", "mMTC"] = Field(
        ...,
        description="Tipo de serviço/slice"
    )
    intent: str = Field(
        ...,
        description="Texto em linguagem natural"
    )
    sla_requirements: Optional[Dict[str, Any]] = Field(
        None,
        description="Requisitos de SLA (opcional, será gerado se não fornecido)"
    )
...
class IntentResponse(BaseModel):
    ...
    semantic_latency_ms: Optional[float] = Field(
        None,
        description="Internal semantic pipeline latency (validate + GST + NEST + metadata), ms",
    )
