...
async def create_intent(
    ...
    if "throughput" in sla_req_dict and isinstance(sla_req_dict["throughput"], (int, float)):
        sla_req_dict["throughput"] = f"{sla_req_dict['throughput']}Mbps"
    # Compatibilidade com payload atual do backend:
    # SLARequirements exige jitter como string (ex.: "2ms"), mas o portal pode enviar número.
    if "jitter" in sla_req_dict and isinstance(sla_req_dict["jitter"], (int, float)):
        sla_req_dict["jitter"] = str(sla_req_dict["jitter"])
    ...
    with tracer.start_as_current_span("process_intent") as span:
        ...
        try:
            ...
            # Garantir latência ML no metadata do HTTP (DE a vezes só espelha em metadata aninhado).
            _resp_meta = dict(decision_response.get("metadata") or {})
            if decision_response.get("ml_prediction_latency_ms") is not None:
                _resp_meta["ml_prediction_latency_ms"] = decision_response[
                    "ml_prediction_latency_ms"
                ]
            if decision_response.get("ml_confidence") is not None:
