...
async def metrics_endpoint_explicit():
    """
    Endpoint Prometheus explícito para evitar regressão de rota 404.
    Mantém compatibilidade com scrapers que consultam GET/HEAD /metrics.
    """
    return Response(generate_latest(), media_type=CONTENT_TYPE_LATEST)
...
async def instantiate_nsi(nsi_spec: dict):
    ...
    with tracer.start_as_current_span("instantiate_nsi") as span:
        try:
            ...
            print(
                f"[NSI_TRACE] nsiId={nsi_id} tenant_id={nsi_spec.get('tenant_id')} "
                f"spec_nsiId={nsi_spec.get('nsiId')}",
                flush=True,
            )
            logger.info(
                "✅ [NSI] NSI instanciado nsiId=%s tenant_id=%s spec_nsiId=%s",
                nsi_id,
                nsi_spec.get("tenant_id"),
                nsi_spec.get("nsiId"),
            )
