...
Modo DEV: BC_ENABLED=false usa stub sem blockchain
"""
...
if not BC_ENABLED:
    print("⚠️ Blockchain DESABILITADO — usando stub BCClient")
    
    # Criar stub BCClient para modo DEV
    class BCClient:
        """Stub BCClient para modo DEV quando blockchain está desabilitado"""
        def __init__(self):
            print("   [STUB] BCClient inicializado em modo offline")
            self.w3 = None
            self.contract = None
            self.contract_address = None
            self.account = None
        
        def write_sla_decision(self, *args, **kwargs):
            """Stub: retorna resposta simulada"""
            return {"status": "offline", "tx": None, "message": "Blockchain desabilitado (modo DEV)"}
        
        def get_sla_status(self, *args, **kwargs):
            """Stub: retorna status simulado"""
            return {"status": "offline", "message": "Blockchain desabilitado (modo DEV)"}
        
        async def register_sla_on_chain(self, *args, **kwargs):
            """Stub: retorna None (não registra)"""
            return None
        
        async def update_sla_status(self, *args, **kwargs):
            """Stub: retorna None (não atualiza)"""
            return None
        
        def get_sla_from_chain(self, *args, **kwargs):
            """Stub: retorna None"""
            return None
        
        def is_enabled(self):
            """Retorna False em modo stub"""
            return False
    
    # Exportar stub e terminar (não carregar código blockchain real)
    tracer = trace.get_tracer(__name__)
    
else:
    # Modo PRODUÇÃO: carregar código blockchain real
    try:
        ...
        Web3 = None
        print("⚠️ web3 não está instalado. BCClient em modo degraded.")
    ...
    class BCClient:
        ...
        def is_enabled(self):
            """Retorna True se blockchain está disponível"""
