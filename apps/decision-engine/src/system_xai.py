...
def explain_decision(snapshot: Dict[str, Any], decision_result: DecisionResult) -> Dict[str, Any]:
    ...
    md = decision_result.metadata if isinstance(decision_result.metadata, dict) else {}
    ds = md.get("decision_score")
    cf = md.get("contributing_factors")
    if ds is not None and isinstance(cf, list) and cf:
        top = sorted(
            (x for x in cf if isinstance(x, dict)),
            key=lambda x: float(x.get("contribution") or 0.0),
            reverse=True,
        )[:4]
        names = [str(x.get("factor")) for x in top]
        extra = (
            f" [decision_score={float(ds):.3f}] Fatores que mais elevaram o score: "
            f"{', '.join(names)}."
        )
        explanation["explanation"] = (explanation.get("explanation") or "").strip() + extra
