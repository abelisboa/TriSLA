...
class AgentTransport:
    ...
    def __init__(
        ...
        agent_id: str = "agent-transport-1",
        ...
        with tracer.start_as_current_span("execute_transport_action") as span:
            ...
            if self.nasp_client:
                try:
                    ...
                    # Usar action_executor ou método genérico do NASP Client
                    sys.path.insert(0, os.path.join(
                        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
                        "nasp-adapter",
                        "src"
                    ...
                    action_with_domain = {**action, "domain": "transport"}
    ...
    async def run_autonomous_loop(self):
        ...
        self.running = True
        logger.info(f"🔄 Iniciando loop autônomo do Agent Transport (intervalo: {self.poll_interval}s)")
    ...
    def stop_autonomous_loop(self):
        ...
        self.running = False
        logger.info("🛑 Parando loop autônomo do Agent Transport")
