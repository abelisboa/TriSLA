...
def rpc_connected() -> bool:
    try:
        conn = bool(w3().is_connected())
        if not conn:
            logger.error("[BC] RPC not connected (web3.is_connected() == False)")
        return conn
    except Exception as e:
        logger.error("[BC] RPC connectivity check failed: %s", e)
        return False
...
def build_and_send_signed_tx(
    ...
    if not web3.is_connected():
        logger.error("[BC] send_raw_transaction aborted: web3.is_connected() == False")
    ...
    try:
        ...
        for attempt in range(_MAX_RETRIES):
            ...
            if raw_bytes is None:
                raise RuntimeError("Assinatura sem raw_transaction (eth-account incompatível).")
            try:
                logger.info("[BC] sending transaction to=%s nonce=%s", to, nonce)
                tx_hash = web3.eth.send_raw_transaction(raw_bytes)
                logger.info("[BC] tx_hash=%s", tx_hash.hex())
                break
            except Exception as e:
        ...
        if _BC_TX_RECEIPT_TIMEOUT > 0:
            ...
            if receipt is not None:
                ...
                logger.info("[BC] receipt_status=%s block=%s", st, receipt.blockNumber)
                if st != 1:
                    raise RuntimeError(
                        f"Transaction reverted on-chain (receipt.status={st}) tx={tx_hash.hex()}"
                    )
