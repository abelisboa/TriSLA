...
class BCService:
    ...
    def __init__(self):
        ...
        try:
            ...
            if not self.w3.is_connected():
                logger.error(
                    "[BC] Não conectado ao Besu RPC em %s (web3.is_connected() == False). Modo degraded.",
    ...
    def _require_success_receipt(self, receipt, context: str):
        if receipt is None:
            raise BCInfrastructureError(f"{context}: receipt ausente")
        st = int(receipt.status)
        if st != 1:
            raise BCInfrastructureError(
                f"{context}: transação revertida (receipt.status={st}) tx={receipt.transactionHash.hex()}"
            )

    def _finalize_signed_tx_receipt(self, tx_result: dict, context: str):
        """Usa receipt do tx_sender quando disponível; senão aguarda confirmação única."""
        receipt = tx_result.get("receipt")
        tx_hash_hex = tx_result["tx_hash"]
        if receipt is None:
            receipt = self.w3.eth.wait_for_transaction_receipt(tx_hash_hex)
        self._require_success_receipt(receipt, context)
        logger.info(
            "[BC] receipt_status=%s tx=%s",
            int(receipt.status),
    ...
    def register_sla(self, customer, service_name, sla_hash, slos):
        ...
        try:
            ...
            if WALLET_AVAILABLE and load_account:
                ...
                tx_data = self.contract.functions.registerSLA(
                    customer, service_name, sla_hash, slos
                ...
                receipt = self._finalize_signed_tx_receipt(tx_result, "register_sla")
                logger.info(
                    "SLA registrado (assinatura local): tx_hash=%s, block=%s",
                    receipt.transactionHash.hex(),
                    receipt.blockNumber,
                )
                return receipt
                ...
                self._require_success_receipt(receipt, "register_sla")
    ...
    def update_status(self, sla_id, status):
        ...
        try:
            ...
            if WALLET_AVAILABLE and load_account:
                ...
                tx_data = self.contract.functions.updateSLAStatus(sla_id, status)._encode_transaction_data()
                ...
                receipt = self._finalize_signed_tx_receipt(tx_result, "update_status")
                logger.info(
                    "Status SLA %s atualizado (assinatura local): tx_hash=%s, status=%s",
                    sla_id,
                    receipt.transactionHash.hex(),
                    status,
                )
                ...
                receipt = self.w3.eth.wait_for_transaction_receipt(tx)
                self._require_success_receipt(receipt, "update_status")
