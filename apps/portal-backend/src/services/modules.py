...
from src.utils.prometheus_response import safe_extract_result, safe_first_vector_value
...
class ModuleService:
    ...
    async def get_module(self, module: str) -> dict[str, Any]:
        ...
        try:
            ...
            series = safe_extract_result(result)
            return {
                "name": module,
                "status": "UP" if series else "DOWN",
            }
        except Exception:
    ...
    async def get_module_metrics(self, module: str) -> dict[str, Any]:
        ...
        try:
            ...
            result = await prometheus_service.query(query)
            v = safe_first_vector_value(result)
            if v is not None:
                try:
                    metrics["latency_p95"] = float(v)
