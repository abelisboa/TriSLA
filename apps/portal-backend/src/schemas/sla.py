...
class SLASubmitResponse(BaseModel):
    ...
    decision: str  # ACCEPT | REJECT | RENEGOTIATE
    ...
    # XAI profundo (repassado do SEM-CSMF / Decision Engine)
    reasoning: Optional[str] = None
    confidence: Optional[float] = None
    domains: Optional[list] = None
    metadata: Optional[dict] = None
    semantic_parsing_latency_ms: Optional[float] = None
    sem_csmf_internal_latency_ms: Optional[float] = None
    decision_duration_ms: Optional[float] = None
    admission_time_total_ms: Optional[float] = None
    blockchain_transaction_latency_ms: Optional[float] = None

    # PROMPT_132 — pipeline pós-ACCEPT e telemetria explícitos na resposta
