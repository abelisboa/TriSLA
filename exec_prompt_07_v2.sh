...
if ! curl -s http://localhost:9090/api/v1/targets > /dev/null 2>&1; then
  ...
  echo "Port 9090 already accessible" > "$WORKDIR/01_pf_prometheus.log"
fi
...
curl -s "http://localhost:9090/api/v1/query?query=avg(rate(node_cpu_seconds_total{mode=\"idle\"}[1m]))" > "$WORKDIR/01_q_cpu.json" || true
curl -s "http://localhost:9090/api/v1/query?query=node_memory_MemAvailable_bytes" > "$WORKDIR/01_q_mem.json" || true
curl -s "http://localhost:9090/api/v1/query?query=node_filesystem_avail_bytes" > "$WORKDIR/01_q_disk.json" || true
curl -s "http://localhost:9090/api/v1/query?query=rate(container_network_transmit_bytes_total[1m])" > "$WORKDIR/01_q_net.json" || true
...
oks = sum(ok("$WORKDIR/" + f) for f in files)
...
# FASE 3
grep -Rni "xai_data" . | tee "$WORKDIR/03_xai_grep.txt" || true
...
if [ -f "$WORKDIR/04_intent_accept.txt" ]; then
  ...
  kubectl run -n $NS kafkatools --rm -i --restart=Never --image=bitnami/kafka:3.6.0 -- \
    bash -lc "kafka-topics.sh --bootstrap-server kafka:9092 --list" > "$WORKDIR/05_topics.txt" || true
...
kubectl logs -n $NS "$BC_POD" --since=30m > "$WORKDIR/06_bc_logs.txt" || true
kubectl logs -n $NS "$BESU_POD" --since=30m > "$WORKDIR/06_besu_logs.txt" || true
...
kubectl logs -n $NS "$SEM_POD" --since=30m > "$WORKDIR/07_sem_logs.txt" || true
grep -nEi "gst|nest|ontology|reasoner|semantic" "$WORKDIR/07_sem_logs.txt" | head -n 200 > "$WORKDIR/07_sem_hits.txt" || true

kubectl logs -n $NS "$ML_POD" --since=30m > "$WORKDIR/07_ml_logs.txt" || true
...
# FASE 8
sha256sum $(find "$WORKDIR" -type f | sort) > "$WORKDIR/CHECKSUMS.sha256" || true
