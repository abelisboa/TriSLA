#!/usr/bin/env python3
"""Sprint 5M4 E2E — runtime closed-loop assurance validation."""
...
def assurance_populated(obj: dict | None) -> bool:
    if not obj or not isinstance(obj, dict):
        return False
    return bool(obj.get("state") or obj.get("assurance_state"))
...
def main() -> int:
    ...
    for key, intent_text, template_id in INTENTS:
        tenant = f"trisla-5m4-{uuid.uuid4().hex[:8]}"
        ...
        submit_meta = submit.get("metadata") or {}
        submit_assurance = submit_meta.get("runtime_assurance")
        status_assurance = status.get("runtime_assurance")
        sla_ingest = submit_meta.get("sla_agent_ingest") or {}
        ...
        runtime_extract = {
            ...
            "governance_event_id": submit_meta.get("governance_event_id")
            or ((submit_meta.get("governance_event") or {}).get("governance_event_id")),
            "governance_event_nest_id": ((submit_meta.get("governance_event") or {}).get("nest_id")),
            "submit_runtime_assurance_populated": assurance_populated(submit_assurance),
            "status_runtime_assurance_populated": assurance_populated(status_assurance),
            "sla_agent_runtime_assurance": sla_ingest.get("runtime_assurance"),
            "submit_runtime_assurance": submit_assurance,
            "status_runtime_assurance": status_assurance,
        }
        (OUT / f"runtime_assurance_{key}.json").write_text(
        ...
        ok = (
            ...
            and (
                runtime_extract["submit_runtime_assurance_populated"]
                or runtime_extract["status_runtime_assurance_populated"]
            )
        )
        ...
        if not ok:
            failures.append(key)
        print(
            f"[E2E] {key}: submit_assurance={runtime_extract['submit_runtime_assurance_populated']} "
            f"status_assurance={runtime_extract['status_runtime_assurance_populated']} pass={ok}",
            flush=True,
        )
        time.sleep(2)
