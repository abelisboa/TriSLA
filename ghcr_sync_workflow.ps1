...
# 2️⃣ Comparar com imagens do Híbrido (backup mais recente)
Write-Host "`n🔍 Verificando backup Híbrido..." -ForegroundColor Yellow
...
if (Test-Path $backupPath) {
    ...
    $hibridoImages = Get-Content $backupPath | Select-String "ghcr.io"
    if ($hibridoImages) {
        Write-Host "Imagens Híbrido (backup):" -ForegroundColor Green
        $hibridoImages | ForEach-Object { Write-Host $_.Line -ForegroundColor White }
    }
} else {
...
Write-Host "✅ TriSLA local e Híbrido estão 100% alinhados!" -ForegroundColor Green
