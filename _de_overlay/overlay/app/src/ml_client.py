    ...
    async def predict_viability(self, decision_input: DecisionInput) -> Optional[MLPrediction]:
        ...
        with tracer.start_as_current_span("predict_viability_ml") as span:
            ...
            try:
                ...
                # VALIDAÇÃO XAI OBRIGATÓRIO: Se model_used=true, explanation é obrigatório
                explanation_data = data.get("explanation", {})
                explanation_reasoning = explanation_data.get("reasoning") if explanation_data else None
                
                if model_used and (not explanation_data or not explanation_reasoning or explanation_reasoning.strip() == ""):
                    error_msg = f"XAI explanation missing: model_used=true mas explanation ausente ou vazio para intent_id={decision_input.intent.intent_id}"
                    logger.error(f"❌ {error_msg}")
                    span.set_attribute("ml.xai_validation", "failed")
                    span.set_attribute("ml.error", error_msg)
                ...
                if not model_used:
                    ...
                    logger.info(f"✅ XAI explanation presente: {explanation_reasoning[:100] if explanation_reasoning else 'N/A'}")
                    span.set_attribute("ml.xai_validation", "passed")
                ...
                prediction = MLPrediction(
                    ...
                    explanation=explanation_reasoning,  # XAI obrigatório validado acima
