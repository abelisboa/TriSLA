...
async def evaluate_sla(sla_input: SLAEvaluateInput):
    ...
    with tracer.start_as_current_span("evaluate_sla") as span:
        try:
            ...
            # Logar explanation explicitamente por decision_id
            if decision_result.explanation:
                logger.info(f"🔍 XAI explanation para decision_id={decision_result.decision_id}: {decision_result.explanation[:200]}")
            else:
                logger.warning(f"⚠️ XAI explanation ausente para decision_id={decision_result.decision_id}")
