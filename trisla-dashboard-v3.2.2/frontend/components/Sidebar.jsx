export default function Sidebar({ status }){
  ...
  return (
    <div style={{width:"260px", padding:"20px", background:"#0a1a38", borderRight:"1px solid #102a5c"}}>
      ...
      {item("Criar Slice (NPL)", on(status.sem_nsmf))}
      {item("Criar Slice (GST)", on(status.sem_nsmf))}
      {item("Métricas", on(status.prometheus))}
      <div style={{marginTop:"14px", paddingTop:"10px", borderTop:"1px dashed #1f3466"}}>
        ...
        {badge("Prometheus", on(status.prometheus))}
        {badge("SEM-NSMF", on(status.sem_nsmf))}
      </div>
    </div>
  );
}